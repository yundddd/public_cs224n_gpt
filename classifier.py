...
def model_test_eval(dataloader, model, device):
    ...
    sent_ids = []
    for step, batch in enumerate(tqdm(dataloader, desc=f'eval', disable=TQDM_DISABLE)):
        b_ids, b_mask, b_sents, b_sent_ids = batch['token_ids'], batch[
            'attention_mask'], batch['sents'], batch['sent_ids']
...
def train(args):
    ...
    for epoch in range(args.epochs):
        ...
        num_batches = 0
        for batch in tqdm(
                train_dataloader, desc=f'train-{epoch}', disable=TQDM_DISABLE):
