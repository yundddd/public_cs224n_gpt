...
class ParaphraseDetectionNLPDataset(Dataset):
    def __init__(self, dataset, args):
        self.dataset = dataset
        self.p = args
        self.tokenizer = GPT2Tokenizer.from_pretrained('gpt2')
    ...
    def collate_fn(self, all_data):
        ...
        cloze_style_sents = [
            f'Is "{s1} " a paraphrase of "{s2} "? Answer only "yes" or "no": {label}'
...
class ParaphraseDetectionNLPHoldOutDataset(Dataset):
    def __init__(self, dataset, args):
        self.dataset = dataset
        self.p = args
        self.tokenizer = GPT2Tokenizer.from_pretrained('gpt2')
    ...
    def collate_fn(self, all_data):
        ...
        cloze_style_sents = [
            f'Is "{s1} " a paraphrase of "{s2} "? Answer only "yes" or "no": '
            for (s1, s2) in zip(sent1, sent2)]
        encoding = self.tokenizer(
            cloze_style_sents, return_tensors='pt', padding=True, truncation=True)
