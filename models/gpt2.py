...
class GPT2Model(GPTPreTrainedModel):
    ...
    def __init__(self, config):
        ...
        # GPT-2 layers.
        self.gpt_layers = nn.ModuleList([GPT2Layer(config)
    ...
    @classmethod
    def from_pretrained(cls, model='gpt2', d=768, l=12, num_heads=12, use_flash_attention=False, **kwargs):
        gpt_model = OpenAIGPT2Model.from_pretrained(model).eval()
        # gpt_model = OpenAIGPT2Model.from_pretrained(model, attn_implementation='flash_attention_2').eval()
        our_model = GPT2Model(
            GPT2Config(
        ...
        for i in range(l):
            ...
            l.self_attention.query.weight.data = gpt_model.state_dict()[
                f'h.{i}.attn.c_attn.weight'][:, : d].T
            l.self_attention.query.bias.data = gpt_model.state_dict()[
                f'h.{i}.attn.c_attn.bias'][
                : d]
            l.self_attention.key.weight.data = gpt_model.state_dict()[
                f'h.{i}.attn.c_attn.weight'][:, d: d * 2].T
            l.self_attention.key.bias.data = gpt_model.state_dict()[
                f'h.{i}.attn.c_attn.bias'][d: d * 2]
            l.self_attention.value.weight.data = gpt_model.state_dict()[
                f'h.{i}.attn.c_attn.weight'][:, d * 2:].T
            l.self_attention.value.bias.data = gpt_model.state_dict()[
                f'h.{i}.attn.c_attn.bias'][d * 2:]

            # Remap final dense layer in MHA
            if l.use_lora:
                l.lora_attention_dense.linear.weight.data = gpt_model.state_dict()[f'h.{i}.attn.c_proj.weight'].T
                l.lora_attention_dense.linear.bias.data = gpt_model.state_dict()[f'h.{i}.attn.c_proj.bias']
            else:
            ...
            if l.use_lora:
                l.lora_interm_dense.linear.weight.data = gpt_model.state_dict()[f'h.{i}.mlp.c_fc.weight'].T
                l.lora_interm_dense.linear.bias.data = gpt_model.state_dict()[f'h.{i}.mlp.c_fc.bias']
                l.lora_out_dense.linear.weight.data = gpt_model.state_dict()[f'h.{i}.mlp.c_proj.weight'].T
                l.lora_out_dense.linear.bias.data = gpt_model.state_dict()[f'h.{i}.mlp.c_proj.bias']
            else:
